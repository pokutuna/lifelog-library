...
import com.pokutuna.LifelogDBC.model.SensingModel._
...
class SensingDAOForJava(path: String) {
  ...
  def latestDate: String = dao.latestDate
  ...
  def insertBtDevice(address: String, name: String) = dao.insertBtDevice(address, name)
  def insertBtDevice(device: BtDevice) = dao.insertBtDevice(device)
  def insertWifiDevice(address: String, name: String) = dao.insertWifiDevice(address, name)
  def insertWifiDevice(device: WifiDevice) = dao.insertWifiDevice(device)

  def insertBtDetected(address: String, dateTime: String) = dao.insertBtDetected(address, dateTime)
  def insertBtDetected(detection: BtDetected) = dao.insertBtDetected(detection)
  def insertWifiDetected(address: String, dateTime: String, strength: Int) = dao.insertWifiDetected(address, dateTime, strength)
